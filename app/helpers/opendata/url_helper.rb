module Opendata::UrlHelper
  ...
  def dataset_enabled?
    # node = Opendata::Node::Dataset.site(@cur_site).where(native_id: nil).and_public.first
    # node.present?
  ...
  def app_enabled?
    # node = Opendata::Node::App.site(@cur_site).where(native_id: nil).and_public.first
    # node.present?
    @cur_site = Cms::Site.find(@cur_site.id) if @cur_site.is_a?(SS::Site)
  ...
  def idea_enabled?
    # node = Opendata::Node::Idea.site(@cur_site).where(native_id: nil).and_public.first
    # node.present?
    @cur_site = Cms::Site.find(@cur_site.id) if @cur_site.is_a?(SS::Site)
  ...
  def search_datasets_path(options = {})
    node = Opendata::Node::SearchDataset.site(@cur_site).where(native_id: nil).and_public.first
  ...
  def search_groups_path(options = {})
    node = Opendata::Node::SearchDatasetGroup.site(@cur_site).where(native_id: nil).and_public.first
